...
# accessible /tspro/auth/login with admin/admin — a takeover in one
...
if [[ -f "${ENV_FILE}" ]] && grep -q '^TSP_SECRET_KEY=' "${ENV_FILE}"; then
  ...
  # Pull the existing password back into the shell so the end-of-install
  # banner can print it. On reruns the password is whatever first-boot
  # seeded into the admin account; reading the file is the only way to
  # surface it (we never store it back in the host env).
  if [[ -z "${ADMIN_PASSWORD}" ]] && grep -q '^TSP_ADMIN_PASSWORD=' "${ENV_FILE}"; then
    ADMIN_PASSWORD="$(grep '^TSP_ADMIN_PASSWORD=' "${ENV_FILE}" | head -1 | cut -d= -f2-)"
  fi
else
  log "Generating new TSP_SECRET_KEY"
  SECRET_KEY="$(openssl rand -base64 48 | tr -d '\n/+=' | cut -c1-64)"
  if [[ -z "${ADMIN_PASSWORD}" ]]; then
    log "Generating random TSP_ADMIN_PASSWORD (printed at end of run)"
